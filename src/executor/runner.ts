...
async function executeWithRetry(page: Page, intent: Intent): Promise<StepResult> {
  const stepStart = Date.now();
  for (let attempt = 1; attempt <= RETRY_LIMIT; attempt += 1) {
    try {
      const parsedIntent = intentSchema.parse({ ...intent });
...
export async function runIntents(intents: Intent[], options: RunnerOptions = {}): Promise<RunResult> {
  ...
  for (const intent of intents) {
    ...
    const result = await executeWithRetry(page, intent);
    steps.push(result);
